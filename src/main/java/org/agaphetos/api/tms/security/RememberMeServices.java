...
import static org.agaphetos.api.tms.config.SecurityConfig.REMEMBER_ME_KEY;
...
public class RememberMeServices extends AbstractRememberMeServices {
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication successfulAuthentication) {
        ...
        Employee employee = userService.findById(Integer.parseInt(login));
        Token token = new Token();
        token.setSeries(generateSeriesData());
        token.setUserLogin(Integer.toString(employee.getEmployeeId()));
        token.setValue(generateTokenData());
        token.setDate(new Date());
        token.setIpAddress(request.getRemoteAddr());
        token.setUserAgent(request.getHeader("User-Agent"));
    ...
    private Token getPersistentToken(String[] cookieTokens) {
        ...
        Token token = null;
        try {
            token = tokenService.findBySeries(presentedSeries);
        } catch (DataAccessException e) {
